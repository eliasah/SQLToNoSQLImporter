...
public class ESWriter extends NoSQLWriter {
	...
	public void initConnection(ResourceBundle rb) {
		...
		setIndexProperties(rb.getString("es.index.name"),
				rb.getString("es.index.type"));
	}
	...
	public void writeToNoSQL(List<Map<String, Object>> entityList)
		...
		if (bulkRequest.numberOfActions() > 0) {
			...
			for (Iterator<BulkItemResponse> iterator = response.iterator(); iterator
					.hasNext();) {
				BulkItemResponse e = (BulkItemResponse) iterator.next();
				if (e.isFailed())
					throw new FailedCommunicationException(
							"Insertion to ES failed.");
			}
